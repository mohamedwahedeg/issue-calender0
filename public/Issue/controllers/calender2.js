...
angular.module('mwl.calendar.docs') //you will need to declare your module with the dependencies ['mwl.calendar', 'ui.bootstrap', 'ngAnimate']
    .controller('KitchenSinkCtrl', function (moment, alert, calendarConfig, $window, $http, $scope, $rootScope, report_today_data, $location) {
        if ($window.localStorage["user"]) {
            ...
            if (u_level == "1") {
                ...
                success(function (data, status, headers, config) {
                    ...
                    for (i = 0; i <= mylenght; i++) {
                        var s1 = data[i].claim_no;
                        var s2 = data[i].next_session;
                        var s3 = data[i].issue_id;
                        var firstinsert = vm.viewDate.getMonth();
                        firstinsert = firstinsert + 1;
                        if (data[i].next_session == undefined){
                            //
                        }else{
                            var sd = data[i].next_session.split('-');
                            if(sd[1] == firstinsert){
                                $scope.events_v.push({
                                    title: "قضية رقم" + " " + s1
            ...
            //reportsum
            $http.get('/api/reportsum').success(function (data, status, headers, config) {
                $scope.statiscs = data;
                console.log($scope.statiscs);
            }).error(function (data, status, headers, config) {
                $("#loading").hide();
            });
            //
        ...
        ///
            ...
            $scope.filterareaclick = function (valuee) {
                    ...
                    success(function (data, status, headers, config) {
                        $("#loading").hide();
                        $rootScope.mydata_calender_1 = data;
                        $scope.data = data; // ds mean data search
                        //////////
                        var mylenght = $scope.data.length - 1;
                        for (i = 0; i <= mylenght; i++) {
            ...
            $scope.reports0 = function () {
                ...
                ///////////
                $scope.reports2 = function () {
                    $location.path('/reports2');
                }
                ///////////
        }
        else {
...
////////////
        $scope.nextm = function(){
            $scope.events_v = [];
            var dy = vm.viewDate.getFullYear();
            var ndy = new Date();
            if (dy == ndy.getFullYear())
            {
